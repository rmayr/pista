def dtcString(dtcHex):
	dtc = int(dtcHex)
	if (dtc & 0xc000) == 0x0000:
		first = "P"
	elif (dtc & 0xc000) == 0x4000:
		first = "CP"
	elif (dtc & 0xc000) == 0x8000:
		first = "B"
	elif (dtc & 0xc000) == 0xC000:
		first = "U"
				
	second = (dtc & 0x3000) >> 24
	...
	string = "%s%d%s" % (first, second, dtcHex[1:])	
        text = "%s - %s" % (string, dict.get(string, "unknown"))
	return text
